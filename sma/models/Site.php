...
class Site extends CI_Model
{
    ...
    public function syncVariantQty($variant_id, $warehouse_id, $product_id = NULL) {
        ...
        //aqui 3 syncVariantQty
        log_message('info', 'syncVariantQty');
        log_message('info', $product_id);
    ...
    public function getProductComboItemsProd($pid)
    {
        log_message('info', 'getProductComboItemsProd');
        ...
        if ($q->num_rows() > 0) {
            ...
            foreach ($data as $data1) {
                log_message('info', $data1->item_code);
            }
    ...
    public function syncProductQtyCombo($product_id, $warehouse_id,$balance_qty) {
        
        log_message('info', 'syncProductQty//////////////');
        //cris aquiiiiiiiiiiiiiiiiiii
                        
            $wh_balance_qty = $this->getBalanceQuantity($product_id, $warehouse_id);

            $wh_balance_qty = $balance_qty;
            
            if ($this->db->update('products', array('quantity' => $wh_balance_qty), array('id' => $product_id))) {
    ...
    public function syncProductQty($product_id, $warehouse_id,$item) {
        ...
        if ($item->product_type == 'combo') {
            log_message('info', 'warehouses_products COMBO');
            
            log_message('info', 'es un combo');
            
            $combo_items = $this->getProductComboItemsProd($item->product_id);
            foreach ($combo_items as $combo_item) {
                ...
                if ($this->db->update('products', array('quantity' => $balance_qty), array('id' => $product_id))) {
                    if ($this->getWarehouseProducts($product_id, $warehouse_id)) {
                        log_message('info', 'warehouses_products COMBO');
                        log_message('info', $product_id);
                        log_message('info', $warehouse_id);
                        log_message('info', $wh_balance_qty);
                ...
                */
                $pr = $this->getProductByCode($combo_item->item_code);
                $balance_qty = $this->getBalanceQuantityCris($pr->id,$item->warehouse_id);
                
                log_message('info', 'syncProductQty /////////////////////**********4444////////////');
                log_message('info', $item->quantity);
                log_message('info', $balance_qty);
                
                
                $qtyCombo = ($balance_qty - $item->quantity);
                $this->syncProductQtyCombo($pr->id, $item->warehouse_id,$qtyCombo);
            ...
            if ($this->db->update('products', array('quantity' => $balance_qty), array('id' => $product_id))) {
                if ($this->getWarehouseProducts($product_id, $warehouse_id)) {
                    log_message('info', 'warehouses_products standart');
                    log_message('info', $product_id);
                    log_message('info', $warehouse_id);
                    log_message('info', $wh_balance_qty);
    ...
    //cris new 01032020
    private function getBalanceQuantityCris($product_id, $warehouse_id = NULL) {
    ...
    public function calculateAVCost($product_id, $warehouse_id, $net_unit_price, $unit_price, $quantity, $product_name, $option_id, $item_quantity) {
        ...
        if ($quantity > 0 && !$this->Settings->overselling) {
            ...
            array_push($cost, array('date' => date('Y-m-d'), 'product_id' => $product_id, 'sale_item_id' => 'sale_items.id', 'purchase_item_id' => NULL, 'quantity' => $real_item_qty, 'purchase_net_unit_cost' => NULL, 'purchase_unit_cost' => NULL, 'sale_net_unit_price' => $net_unit_price, 'sale_unit_price' => $unit_price, 'quantity_balance' => NULL, 'overselling' => 1, 'inventory' => 1));
            array_push($cost, array('pi_overselling' => 1, 'product_id' => $product_id, 'quantity_balance' => (0 - $quantity), 'warehouse_id' => $warehouse_id, 'option_id' => $option_id));
        }
        return $cost;
    }

    public function calculateCostCombo($product_id, $warehouse_id, $net_unit_price, $unit_price, $quantity, $product_name, $option_id, $item_quantity) {
    ...
    public function calculateCost($product_id, $warehouse_id, $net_unit_price, $unit_price, $quantity, $product_name, $option_id, $item_quantity) {
        ...
        foreach ($pis as $pi) {
            if (!empty($pi) && $balance_qty <= $quantity && $quantity > 0) {
                ...
                if ($pi->quantity_balance >= $quantity && $quantity > 0) {
                    $balance_qty = $pi->quantity_balance - $quantity;
                    $cost_row = array('date' => date('Y-m-d'), 'product_id' => $product_id, 'sale_item_id' => 'sale_items.id', 'purchase_item_id' => $pi->id, 'quantity' => $real_item_qty, 'purchase_net_unit_cost' => $pi->net_unit_cost, 'purchase_unit_cost' => $purchase_unit_cost, 'sale_net_unit_price' => $net_unit_price, 'sale_unit_price' => $unit_price, 'quantity_balance' => $balance_qty, 'inventory' => 1, 'option_id' => $option_id, 'sale_id' => '');
                    $quantity = 0;
                } elseif ($quantity > 0) {
                    $quantity = $quantity - $pi->quantity_balance;
                    $balance_qty = $quantity;
                    $cost_row = array('date' => date('Y-m-d'), 'product_id' => $product_id, 'sale_item_id' => 'sale_items.id', 'purchase_item_id' => $pi->id, 'quantity' => $pi->quantity_balance, 'purchase_net_unit_cost' => $pi->net_unit_cost, 'purchase_unit_cost' => $purchase_unit_cost, 'sale_net_unit_price' => $net_unit_price, 'sale_unit_price' => $unit_price, 'quantity_balance' => 0, 'inventory' => 1, 'option_id' => $option_id,'sale_id' => '');
                }
            }
            array_push($cost, $cost_row);
            if ($quantity == 0) {
    ...
    //retorna los items del combo
    public function getProductComboItems($pid, $warehouse_id = NULL)
    {
        log_message('info', 'getProductComboItems');
        ...
        if ($q->num_rows() > 0) {
            ...
            foreach ($data as $data1) {
                log_message('info', $data1->product_id);
            }
        ...
        if ($this->Settings->accounting_method != 2 && !$this->Settings->overselling) {
            ...
            if ($this->site->getProductByID($item['product_id'])) {
                if ($item['product_type'] == 'standard') {
                    ...
                    log_message('info', 'item_costing 333333 //////////////////////////////');
                    log_message('info', $item['product_id']);
                    $combo_items = $this->getProductComboItemsProd($item['product_id']);
                    
                    foreach ($combo_items as $combo_item) {
                        $pr = $this->getProductByCode($combo_item->item_code);
                        log_message('info', 'productos code $combo_item->qty');
                        log_message('info', $pr->id);
                        
                        //$qtyCombo = ($combo_item->qty - $item['quantity']);
                        //$this->syncProductQtyCombo($pr->id, $item['warehouse_id'],$qtyCombo);
                        
                        //$this->syncQuantity(NULL, NULL, NULL, $pr->id);
                        ...
                        $cost[] = $this->calculateCostCombo($pr->id, $item['warehouse_id'], $net_unit_price, $unit_price, ($combo_item->qty * $item['quantity']), $pr->name, NULL, $item_quantity);
            ...
            if ($this->site->getProductByID($item['product_id'])) {
                if ($item['product_type'] == 'standard') {
                    array_push($cost, $this->calculateAVCost($item['product_id'], $item['warehouse_id'], $item['net_unit_price'], $item['unit_price'], $item['quantity'], $item['product_name'], $item['option_id'], $item_quantity));
                } elseif ($item['product_type'] == 'combo') {
                    ...
                    foreach ($combo_items as $combo_item) {
                        $combo_item = $this->getProductByCode($combo_item->item_code);
                        
                        array_push($cost , $this->site->calculateAVCost($combo_item->id, $item['warehouse_id'], ($combo_item->qty * $item['quantity']), $item['net_unit_price'], $item['unit_price'], $item['quantity'], $item['product_name'], $item['option_id'], $item_quantity));
                    }
    ...
    public function syncQuantity($sale_id = NULL, $purchase_id = NULL, $oitems = NULL, $product_id = NULL) {
        if ($sale_id) {
            ...
            log_message('info', "syncQuantity $sale_id");
            $sale_items = $this->getAllSaleItems($sale_id);
                        
            foreach ($sale_items as $item) {
                $this->syncProductQty($item->product_id, $item->warehouse_id,$item);
            ...
            foreach ($purchase_items as $item) {
                log_message('info', $item->product_id +", "+$item->warehouse_id);
            ...
            foreach ($warehouses as $warehouse) {
                ...
                $this->syncProductQty($product_id, $warehouse->id,$item);
    ...
    public function getAllSaleItems($sale_id) {
        log_message("info", 'sale_id: '+ $sale_id);
    ...
    public function syncPurchaseItems($data = array()) {
        ...
        //aqui sync purchase items 
        
        log_message('info', 'syncPurchaseItems');
        //$this->sma->print_arrays($data);
        
        if (!empty($data)) {
            foreach ($data as $items) {
                foreach ($items as $item) {
                    log_message('info', 'syncPurchaseItems213213123123123123123');
                    if (isset($item['pi_overselling'])) {
                        ...
                        if ($pi = $this->site->getPurchasedItem($clause)) {
                            ...
                            log_message('info', 'syncPurchaseItems//////////////////// 3333333333333333');
                            $clause['quantity'] = 0;
                            $clause['item_tax'] = 0;
                        ...
                        if ($item['inventory']) {
                            log_message('info', 'syncPurchaseItems//////////////////// purchase_items');
                            log_message('info', $item['quantity_balance']);
                            log_message('info', $item['purchase_item_id']);
